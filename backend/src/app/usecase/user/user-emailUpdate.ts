...
import { EmailUpdate } from 'src/core/interfaces/user';
...
export class UserEmailUpdate {
  ...
  async exec({ email, id, password }: EmailUpdate): Promise<void> {
    const user = await this.iUserRepository.findById(id);
    if (user?.get.email == email) throw new ApiException(ERROR_MESSAGES[ERROR_CODES.INVALID_EMAIL], HttpStatus.BAD_REQUEST, ERROR_CODES.INVALID_EMAIL);
